...
- (void) loadProcedures
{
    ...
    NSUInteger count = nil; //[ICD10Procedure MR_countOfEntities];
    if (count == 0)
    {
    ...
    count = nil; //[ICD10ProcedureIndex MR_countOfEntities];
    if (count == 0)
    {
        [self loadICD10Index];
    }

    count = nil; //[ICD10ProcedureDefinition MR_countOfEntities];
    if (count == 0)
    {
...
- (NSArray*) findProceduresWithParentCode:(NSString*)parentCode inElement:(TFHppleElement*) element
{
    ...
    for (NSDictionary *dict in arrLabels)
    {
        ...
        ICD10Procedure *proc = nil; //[ICD10Procedure MR_findFirstWithPredicate:[NSPredicate predicateWithFormat:@"%K == %@ AND %K == %@ AND %K == %@", @"key", key, @"code", code, @"name", name]];
        if (!proc)
        {
            proc = nil; //[ICD10Procedure MR_createEntity];
...
-(void) updateProcedureWithCode:(NSString*) code withShortName:(NSString*) shortName withLongName:(NSString*) longName
{
    ...
    ICD10Procedure *proc = nil; //[ICD10Procedure MR_findFirstWithPredicate:predicate];
...
- (ICD10ProcedureIndex*) findIndexInElement:(TFHppleElement*) element
{
    ...
    ICD10ProcedureIndex *index = nil; //[ICD10ProcedureIndex MR_findFirstWithPredicate:predicate];
    
    if (!index)
    {
        index = nil; //[ICD10ProcedureIndex MR_createEntity];
...
- (ICD10Procedure*) findProcedureInElement:(TFHppleElement*) element
{
    ...
    for (TFHppleElement *child in element.children)
    {
        if ([child.tagName isEqualToString:@"tab"] ||
            ...
            [child.tagName isEqualToString:@"codes"])
        {
            NSString *content = nil; //[[child firstChild] content];
            
            proc = nil; //[ICD10Procedure MR_findFirstByAttribute:@"code" withValue:content];
            break;
        }
    }
